...
const EAR_CONSEC_FRAMES = 1; // Lowered to catch even very quick blinks (1 frame = ~33ms at 30fps)
const MAR_THRESHOLD = 0.65; // Lowered threshold to catch smaller yawns
const MAR_CONSEC_FRAMES = 8; // Lowered to catch shorter yawns (8 frames = ~267ms at 30fps)
...
export default function WebcamFeed({ 
  ...
  const eyeState = useRef({ 
    framesClosed: 0, 
    blinkStartTime: 0,
    wasClosed: false, // Track previous state to detect transitions
    blinkDetected: false // Prevent double-counting
  });
  const yawnState = useRef({ 
    framesOpen: 0, 
    yawnStartTime: 0,
    wasOpen: false, // Track previous state to detect transitions
    yawnDetected: false // Prevent double-counting
  });
      ...
      if (results.faceLandmarks?.length) {
        ...
        if (isMonitoring || isCalibrating) {
            if (isMonitoring) { // Only calculate blinks/yawns if monitoring
                ...
                // Blinks are detected when eyes close and then reopen
                const isEyesClosed = avgEAR < EAR_THRESHOLD;
                
                if (isEyesClosed) {
                    // Eyes are closed - track duration
                    eyeState.current.framesClosed++;
                    if (eyeState.current.framesClosed === 1) {
                        eyeState.current.blinkStartTime = performance.now();
                    }
                    eyeState.current.wasClosed = true;
                } else {
                    // Eyes are open now
                    if (eyeState.current.wasClosed) {
                        // We just transitioned from closed to open
                        if (eyeState.current.framesClosed >= EAR_CONSEC_FRAMES && !eyeState.current.blinkDetected) {
                            // Eyes were closed for enough frames = BLINK DETECTED
                            const blinkDuration = (performance.now() - eyeState.current.blinkStartTime) / 1000;
                            newMetrics = {...newMetrics, blinkCount: 1, blinkDuration };
                            eyeState.current.blinkDetected = true; // Prevent double-counting
                        }
                    }
                    // Reset state when eyes are open
                    eyeState.current.framesClosed = 0;
                    eyeState.current.wasClosed = false;
                    eyeState.current.blinkDetected = false;
                }
                
                // === IMPROVED YAWN DETECTION ===
                // Yawns are detected when mouth opens wide and stays open for sufficient time
                const isMouthOpen = mar > MAR_THRESHOLD;
                
                if (isMouthOpen) {
                    // Mouth is open - track duration
                    yawnState.current.framesOpen++;
                    if (yawnState.current.framesOpen === 1) {
                        yawnState.current.yawnStartTime = performance.now();
                    }
                    
                    // Detect yawn while mouth is still open (once threshold is met)
                    if (yawnState.current.framesOpen >= MAR_CONSEC_FRAMES && !yawnState.current.yawnDetected) {
                        // Mouth has been open long enough = YAWN DETECTED
                        const yawnDuration = (performance.now() - yawnState.current.yawnStartTime) / 1000;
                        newMetrics = {...newMetrics, yawnCount: 1, yawnDuration };
                        yawnState.current.yawnDetected = true; // Prevent double-counting
                    }
                    yawnState.current.wasOpen = true;
                } else {
                    // Mouth is closed now
                    // If we detected a yawn while open, it's already counted
                    // Just reset the state
                    yawnState.current.framesOpen = 0;
                    yawnState.current.wasOpen = false;
                    yawnState.current.yawnDetected = false;
                }
            }
